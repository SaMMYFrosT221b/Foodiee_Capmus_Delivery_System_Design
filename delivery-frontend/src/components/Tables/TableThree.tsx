  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/deliveryboy/live-delivery-orders`);
        ...
        if (!ignore) {
          // const packageItems = fetched_data.items.map(item = >{})
          
          setdata(fetched_data[0]['items']);
          setuniqeUSERID(fetched_data[0]['items']['distinctUser']);
